...
class Evencal_model extends CI_Model {
 ...
 function getEvent($year, $month, $day){
  ...
  $query = $this->db->select('event_id as id, event_time as time, event')->order_by('event_time')->get_where('event_detail', array('event_date' => $year));
  if($query->num_rows() > 0){
 ...
 function deleteEvent($year, $month, $day, $id){
  $this->db->delete("event_detail", array('event_id' => $id, 'event_date' => "$year-$month-$day"));
